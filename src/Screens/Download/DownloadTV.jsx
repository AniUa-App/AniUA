...
export default function DownloadTV({ isNavBarPadding, hasManualHeader }) {
  ...
  const getInfos = useCallback(() => {
    try {
      ...
      Logger.error("DownloadTV", "Помилка при завантаженні інформації", error);
      setInfo({});
    }
  }, []);

  const updateInfo = useCallback((slug, newInfoData) => {
    setInfo((prevInfo) => {
      const updatedInfo = {
        ...prevInfo,
  ...
  const fetchAnimeDetails = useCallback(async (animeSlug) => {
    try {
      ...
      Logger.error("DownloadTV", "Помилка при отриманні деталей аніме", {
        animeSlug,
        error,
      });
      return null;
        ...
        <View style={[styles.emptyContainer, { marginTop: -height * 0.1 }]}>
          ...
          <Text
            ...
            style={[
              styles.emptyMessage,
              H2,
              { color: themeColors.inActiveText, fontSize: 24 },
            ]}
          >
      ...
      {selectedAnime && (
        <BottomSheetDownload
          ...
          onDownloadComplete={(episode) => {
            Logger.info("DownloadTV", "Download complete", { episode });
          }}
          onDownloadError={(error, episode) => {
            Logger.error("DownloadTV", "Download error", { error, episode });
          }}
        />
      )}
    </DefaultScreenWidget>
