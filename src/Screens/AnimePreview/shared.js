...
export function useAnimePreview({ route, navigation }) {
  ...
  const analyticsLoggedForSlug = useRef(null);
    ...
    const fetchData = async () => {
      try {
        ...
        const fetchWatchStatus = async () => {
          let resolvedIsFavoriteHikka = false;
          ...
          if (
            isMounted &&
            _fromTap &&
            analyticsLoggedForSlug.current !== anime.slug
          ) {
            analyticsLoggedForSlug.current = anime.slug;
            const title =
              anime.title_ua || anime.title_en || anime.title_ja || "";
            const genreNames = (anime.genres || [])
              .map((g) => g.name_ua || g.name_en)
              .filter(Boolean);
            AnalyticsService.logAnimeOpen(
              anime.slug,
              title,
              genreNames,
              resolvedIsFavoriteHikka,
            );
          }
        };
        ...
        const fetchEpisodesAndDubbings = async () => {
          if (
            ...
            try {
              ...
              try {
                const aniuaData =
                  await AniuaApi.getAnimeEpisodesGroupedByPlayer(anime.slug);
              ...
              if (needFallback) {
                ...
                var result = null;
                try {
                  result = await HikkaApiComplete.getAnimeEpisodes(anime.slug);
                  ...
                  if (!result.error && result.code < 400 && result.data) {
                    ...
                    Logger.info(
                      "shared.js",
                      "Successfully loaded episodes from HikkaApiComplete fallback",
                    );
                  } else {
                    fallbackFailed = true;
                  }
                  ...
                  Logger.error(
                    "shared.js",
                    "HikkaApiComplete fallback also failed",
                    hikkaError,
                  );
                  fallbackFailed = true;
                }
  ...
  const handleEpisodeSelect = useCallback(
    (item) => {
      ...
      AnalyticsService.logEpisodeStart(
        anime.slug,
        item.episode,
        info.dub_team,
        info.player === "Вбудований плеєр" ? "builtin" : "web",
        info.player === "Вбудований плеєр" ? "builtin" : info.player,
      );
