...
// Додаємо плагіни
reactotron.use(apisaucePlugin()).use(trackGlobalErrors());

// Додаємо плагін MMKV тільки якщо екземпляр існує і має потрібні методи
if (
  mmkvInstance &&
  typeof mmkvInstance.addOnValueChangedListener === "function"
) {
  reactotron.use(
    mmkvPlugin({
  ...
  console.warn(
    "ReactotronConfig",
    "MMKV інстанс не містить методу addOnValueChangedListener - плагін MMKV не буде підключено"
  );
}
...
if (__DEV__) {
  console.debug("ReactotronConfig", "MMKV інстанс", {
    available: mmkvInstance ? "доступний" : "недоступний",
  });
}
